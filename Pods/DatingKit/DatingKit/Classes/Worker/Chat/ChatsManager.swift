...
//        close(task: task,
//              with: ChatListModifidResult(modifit: .connected,
//                                          operationState: .succses,
//                                          itemsList: [ChatItem](),
//                                          updated: [IndexPath](),
//                                          added: [IndexPath](),
...
//        self.cacheTool.observeToChatList { (changes) in
//            switch changes {
//
//            case .initial(_):
//
...
//                var modification: ChatListModifidState = .none
//                let deleted: [IndexPath] = deletions.map({ IndexPath(row: $0, section: 0)})
//                let updated: [IndexPath] = modifications.map({ IndexPath(row: $0, section: 0) })
//                let added: [IndexPath] = insertions.map({ IndexPath(row: $0, section: 0) })
//
//                if deleted.count > 0 {
...
//            }
//
//            close(task: task, with: ChatListResult(itemsList: list, status: .succses), task: .done)
//            return
//        }
